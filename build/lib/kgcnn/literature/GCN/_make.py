...
from kgcnn.layers.gather import GatherState, LazyConcatenate
...
def make_model_weighted(inputs: list = None,
    ...
    if use_graph_state:
        model = ks.models.Model(
            inputs=[node_input, edge_input, edge_index_input, node_weights_input, graph_state_input],
            outputs=out, name=name)
    else:

        model = ks.models.Model(
            inputs=[node_input, edge_input, edge_index_input, node_weights_input],
            outputs=out, name=name)
