...
def make_model(inputs: list = None,
    ...
    if use_graph_state:
        model = ks.models.Model(
            inputs=[node_input, edge_input, edge_index_input, graph_state_input],
...
def make_crystal_model(inputs: list = None,
    ...
    graph_state_input = ks.layers.Input(**inputs[5]) if use_graph_state else None
    ...
    graph_state = OptionalInputEmbedding(
        **input_embedding["graph"],
        use_embedding=len(inputs[5]["shape"]) < 1)(graph_state_input) if use_graph_state else None
    ...
    if use_graph_state:
        model = ks.models.Model(
            inputs=[node_input, edge_input, edge_index_input, edge_image, lattice, graph_state_input],
            outputs=out, name=name)
    else:
        model = ks.models.Model(
