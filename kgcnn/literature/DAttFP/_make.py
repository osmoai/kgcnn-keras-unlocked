...
from ._dattfp_conv import DAttFPLayer, PoolingNodesDAttFP
...
# Implementation of DAttFP in `tf.keras` - Novel combination:
# "Directed AttentiveFP" - Combining AttentiveFP attention with direction awareness
# 
# Key innovation: Novel combination of:
# - AttentiveFP's molecular attention mechanism (GRU + attention)
...
#
# This is NOT in the literature yet - it's a novel contribution!
...
model_default = {
    ...
    "dattfp_args": {"units": 128, "use_bias": True, "activation": "relu",
                    "attention_units": 64, "use_edge_features": True, "use_gru_update": True},
    "depth": 4,
    "verbose": 10,
...
def make_model(inputs: list = None,
    ...
    r"""Make `DAttFP` graph network via functional API.
    Default parameters can be found in :obj:`kgcnn.literature.DAttFP.model_default`.
    ...
    Args:
        ...
        dattfp_args (dict): Dictionary of layer arguments unpacked in :obj:`DAttFPLayer` layer.
    ...
    # DAttFP layers with direction-aware attention + AttentiveFP mechanism
    print(f"🏗️  Building DAttFP with depth={depth}, direction-aware AttentiveFP attention")
    for i in range(depth):
        if verbose > 5:
            print(f"  Layer {i+1}: Applying DAttFP with forward + backward AttentiveFP attention")
        
        # Apply DAttFP layer with both forward and reverse edges
        n = DAttFPLayer(**dattfp_args)(
            [n, e, edge_index_input, edge_index_reverse_input]
        )

    # Output embedding choice
    if output_embedding == "graph":
        # Use DAttFP-specific pooling
        out = PoolingNodesDAttFP(pooling_method="mean")(n)
    ...
    if use_rms_norm:
        ...
        print(f"🔧 Applied pre-output MLP RMS normalization to DAttFP")
