...
class DGATLayer(ks.layers.Layer):
    """
    Directed Graph Attention Network Layer
    
    Based on "Directed Graph Attention Networks" (ICLR 2019)
    Implements direction-aware attention with separate forward/backward attention mechanisms.
    ...
    def call(self, inputs, training=None):
        ...
        if len(inputs) == 3:
            ...
            raise ValueError(f"DGATLayer expects 3 or 4 inputs, got {len(inputs)}")
...
class PoolingNodesDGAT(PoolingNodes):
    """
    Pooling layer specifically designed for DGAT
    Handles both forward and backward aggregated features
    """
    ...
    def call(self, inputs, **kwargs):
        """
        Pool node features for DGAT
        
        Args:
            inputs: Node features from DGAT layers
            
        Returns:
            Graph-level representation
        """
        return super(PoolingNodesDGAT, self).call(inputs, **kwargs)
