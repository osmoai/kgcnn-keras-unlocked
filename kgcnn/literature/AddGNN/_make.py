...
model_addgnn_pna_default = {
    ...
    "geometric_edge": False, "make_distance": False, "expand_distance": False,
    "gauss_args": {"bins": 20, "distance": 4, "offset": 0.0, "sigma": 0.4},
    "addgnn_pna_args": {"units": 200, "use_bias": True, "activation": "relu",
                        "aggregators": ["mean", "max", "min", "std"],
                        "scalers": ["identity", "amplification", "attenuation"],
                        "delta": 1.0, "dropout_rate": 0.1, "use_edge_features": True, "use_skip_connection": True},
    "use_set2set": True, "depth": 3, "node_dim": 200,
    "set2set_args": {"channels": 32, "T": 3, "pooling_method": "sum", "init_qstar": "0"},
    "pooling_args": {"pooling_method": "segment_sum"},
    "use_graph_state": True,  # Default to True for AddGNN
    "verbose": 10,
    "output_embedding": "graph", "output_to_tensor": True,
...
@update_model_kwargs(model_addgnn_pna_default)
def make_addgnn_pna_model(inputs: list = None,
                         input_embedding: dict = None,
                         geometric_edge: bool = None,
                         make_distance: bool = None,
                         expand_distance: bool = None,
    ...
    r"""Make `AddGNN-PNA` graph network via functional API.
    Default parameters can be found in :obj:`kgcnn.literature.AddGNN.model_addgnn_pna_default`.

    AddGNN-PNA combines AddGNN's additive attention mechanism with PNA's multi-aggregator approach
    for enhanced molecular property prediction.
    ...
    Args:
        name (str): Name of the model. Should be "AddGNN-PNA".
        inputs (list): List of dictionaries unpacked in :obj:`tf.keras.layers.Input`. Order must match model definition.
        input_embedding (dict): Dictionary of embedding arguments for nodes etc. unpacked in :obj:`Embedding` layers.
        geometric_edge (bool): Whether the edges are geometric, like distance or coordinates.
        make_distance (bool): Whether input is distance or coordinates at in place of edges.
        ...
        set2set_args (dict): Dictionary of layer arguments unpacked in :obj:`PoolingSet2SetEncoder` layer.
        pooling_args (dict): Dictionary of layer arguments unpacked in :obj:`PoolingNodes`, `AggregateLocalEdges` layers.
        addgnn_pna_args (dict): Dictionary of layer arguments unpacked in :obj:`GeneralizedPNALayer` layer.
        use_set2set (bool): Whether to use :obj:`PoolingSet2SetEncoder` layer.
        node_dim (int): Dimension of hidden node embedding.
        depth (int): Number of graph embedding units or depth of the network.
        verbose (int): Level of verbosity.
    ...
    # Skip geometric edge handling for simplicity
    # if make_distance:
    #     pos1, pos2 = NodePosition()([ed, edi])
    #     ed = NodeDistanceEuclidean()([pos1, pos2])
    ...
    # AddGNN-PNA convolution layers using generalized PNA
    for i in range(0, depth):
        n = GeneralizedPNALayer(**addgnn_pna_args)([n, ed, edi])
    ...
    if output_embedding == 'graph':
        ...
        raise ValueError("Unsupported output embedding for mode `AddGNN-PNA`")
