...
# Implementation of rGINE in `tf.keras` extending rGIN with edge features
# Based on: Random Features Strengthen Graph Neural Networks
...
def make_model_edge(inputs: list = None,
    ...
    r"""Make `rGINE` graph network via functional API.
    ...
    Inputs:
        list: `[node_attributes, edge_attributes, edge_indices]`
            ...
            - edge_attributes (tf.RaggedTensor): Edge attributes of shape `(batch, None, F_e)` or `(batch, None)`
              using an embedding layer.
    ...
    Args:
        ...
        rgine_args (dict): Dictionary of layer arguments unpacked in :obj:`rGINE` convolutional layer.
    ...
    e_units = gin_mlp["units"][-1] if isinstance(gin_mlp["units"], list) else int(gin_mlp["units"])
    e = Dense(e_units, use_bias=True, activation='linear')(e)
    
    list_embeddings = [n]
    for i in range(0, depth):
        n = rGINE(**rgine_args)([n, e, edi])
