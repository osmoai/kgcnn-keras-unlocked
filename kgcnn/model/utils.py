...
def get_model_class(module_name: str, class_name: str):
    ...
    try:
        ...
        make_model = getattr(module, class_name)
    except ModuleNotFoundError:
        raise NotImplementedError("Unknown model identifier '%s' for a model in kgcnn.literature." % class_name)
    except AttributeError as e:
        raise NotImplementedError(f"Module '{module_name}' does not have attribute '{class_name}'. Available attributes: {[attr for attr in dir(module) if not attr.startswith('_')]}")
